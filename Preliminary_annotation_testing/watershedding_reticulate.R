...
for(i in seq_along(obb_list)){
  ...
  # Use Python-compatible list for OpenCV call
  img_composite <- py$cv2$polylines(
    img_composite,
    list(pts),
    isClosed = TRUE,
    color = tuple(255L, 0L, 0L),  # red boxes
    thickness = 2L
  )
}

# Render final image in R
library(grid)
img_r <- py_to_r(img_composite) / 255
grid::grid.raster(img_r)
